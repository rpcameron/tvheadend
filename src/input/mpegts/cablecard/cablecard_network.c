 ...
 */
static htsmsg_t *
cablecard_network_config_save(mpegts_network_t *mn, char *filename, size_t fsize)
{
...
{
	cablecard_network_t *cn = calloc(1, sizeof(cablecard_network_t));
...
cablecard_network_init(void)
{
	...
	HTSMSG_FOREACH(f, c) {
		...
		if (!(e = htsmsg_get_map(e, "config")))
			continue;
		cablecard_network_create(f->hmf_name, &cablecard_network_class, e);
	}
